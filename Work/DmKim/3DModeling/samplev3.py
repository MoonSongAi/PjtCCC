...
class MainWindow(QMainWindow):
    ...
    def find_intersection(self, line1, line2, expansion=1):
        ...
        if vertical1 and horizontal2:
            ...
            if (
                x1 >= min(x3, x4) - expansion
                and x1 <= max(x3, x4) + expansion
                and y3 >= min(y1, y2) - expansion
                and y3 <= max(y1, y2) + expansion
            ):
                return (x1, y3)
        elif horizontal1 and vertical2:
            # 선분1이 수평이고 선분2가 수직일 때, 수직선분을 확장합니다.
            if (
                x3 >= min(x1, x2) - expansion
                and x3 <= max(x1, x2) + expansion
                and y1 >= min(y3, y4) - expansion
                and y1 <= max(y3, y4) + expansion
            ):
                return (x3, y1)
    ...
    def remove_near_duplicates(self, intersections, tolerance=5):
        ...
        for current in intersections:
            ...
            if any(
                np.sqrt((x - current[0]) ** 2 + (y - current[1]) ** 2) < tolerance
    ...
    def loadImage(self):
        ...
        if filePath:
            ...
            if img_color is not None:
                ...
                qImg = QImage(
                    img_color_rgb.data,
                    img_color_rgb.shape[1],
                    img_color_rgb.shape[0],
                    bytesPerLine,
                ...
                mask_array = np.frombuffer(ptr, dtype=np.uint8).reshape(
                    (maskQImg.height(), bytes_per_line)
                )
                # Canny 엣지 검출 사용
                ...
                for line in lines:
                    ...
                    if abs(theta) < 92 and abs(theta) > 88:
                        ...
                        print(
                            f"{i} 2theta={theta} x1, y1, x2, y2 = {x1} {y1} {x2} {y2}"
                        )
                    # 각도가 수평선
                ...
                averaged_points = self.average_within_tolerance(
                    unique_intersections, 10
                )
                print(averaged_points)
                for point in averaged_points:
                    x, y = point
                    if (
                        0 <= x < edges.shape[1] and 0 <= y < edges.shape[0]
                    ):  # 이미지 범위 내에 있는지 확인
    ...
    def displayImage(self, qImg, edges):
        ...
        edgesQImg = QImage(
            edges.data, edgesWidth, edgesHeight, bytesPerLine, QImage.Format_Grayscale8
        )
        ...
        ratio = min(
            maxDisplayWidth / (qImgWidth + edgesWidth),
            maxDisplayHeight / max(qImgHeight, edgesHeight),
        )
        ...
        imgLabel.setPixmap(
            QPixmap.fromImage(qImg).scaled(
                newQImgWidth, newQImgHeight, Qt.KeepAspectRatio, Qt.SmoothTransformation
            )
        )
        layout.addWidget(imgLabel)

        maskImgLabel = QLabel()
        maskImgLabel.setPixmap(
            edgesPixmap.scaled(
                newMaskQImgWidth,
                newMaskQImgHeight,
                Qt.KeepAspectRatio,
        ...
        self.imageWindow.setGeometry(
            100,
            100,
            newQImgWidth + newMaskQImgWidth,
            max(newQImgHeight, newMaskQImgHeight),
        )
