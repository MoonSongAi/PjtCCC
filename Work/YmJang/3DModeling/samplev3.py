...
class MainWindow(QMainWindow): # 리네임(상속받을클래스:사실상상위클래스임)
    def __init__(self):
        super().__init__() #super() 

        self.setWindowTitle("Box image rendering") #타이틀
        self.setGeometry(100, 100, 800, 600)       #x,y 어느위치에,어느정도의 사이즈인지
    ...
    def initUI(self):
        ...
        #레이아웃 안에 컴포넌트 컴포넌트 안에 또 컴포넌트:계층구조 > MID프로그램:메인밖으로 벗어나지 않음 ////계층구조 잘 지정해서 조정
        layout = QVBoxLayout(self.centralWidget)
        #앞면을 가지고 제일 긴 선을 height로 나머지 선을 width로 윗면 , 하면 중에 나머지를 깊이로 
    ...
    def handle_click(self, x, y,button_clicked): #self sender를 찾으면 왼쪽 오른쪽 이미지에서 발생했는지 알수있다
        # 이벤트를 발생시킨 객체를 가져옵니다.
        ...
        if sender_label == self.imgLabel:
            ...
            self.toggle_point_in_list(x, y) #toggle 지우기도하고 만들기도 하고 같은 좌표안에 remove 새로운 좌표면 append
            self.update_image_display() #점이 없어지나 생겼을 떄 필요한 업데이트만 시키는 함수  
...
############################################################################
    ...
    def get_intersections(self,lines):
        intersections = []
        for i, line1 in enumerate(lines): #하나의 선을 가지고
            for line2 in lines[i+1:]:      #그 주의의 선을 계속
                intersect = self.find_intersection(line1, line2,10) #붙어 있는 선으로 인식되도록 애매가게 끊기지 않게 10으로 선을 늘림 
    ...
    def draw_points_with_text(self, edges, points):
        ...
        font_scale = 1.5
        #빨간색으로 그렸지만 화면상에는 그레이 스케일 적용되어서 흰검으로만 보임
        font_color = (255, 0, 0)  
        font_thickness = 2
        circle_radius = 20
    ...
    def loadImage(self):
                    #하나만 가지고 오는 것일 거임(멀티 여러게) 모달다이얼 -> 반응을 꼭 해줘야함 행위를 해줘야함 <-> 모달리스다이얼로그
                    #opengl / oepncv
        ...
        if filePath:
            ...
            if img_color is not None:
                ...
                intersections = self.get_intersections(self.selLines) # 1)교차부분을 찾는 
                self.interSections = intersections        #class 변수 저장
                # 교차점 리스트에서 중복 또는 비슷한 위치 제거
                unique_intersections = self.remove_near_duplicates(intersections , 12) #2)근처에 있는 점 하나빼고 제거 /거리 탄젠트12:지름
                # 이미지에 교차점을 그림
                averaged_points = self.average_within_tolerance( unique_intersections, 12) #3)일직선 이여야함 x, y의 값 통일 오차범위 12
                # print(averaged_points)

                text_edges = self.draw_points_with_text(self.mskEdges, averaged_points) # 4)점을 그려주는

                non_overlap_box  = self.find_non_overlapping_boxes(averaged_points) #포인트 4개를 가지고 박스가 되느냐 아니냐/ 오버랩되지 않는 박스 
